...
- Serial：Linux kernel PTY 已纳入 CI；macOS 和 Windows 需要实体设备或已知可靠的虚拟串口。`mark`/`space` parity 已在 Linux `termios2` 和 Windows DCB 路径实现；macOS/其他平台仍由 UI 明确禁用并由后端 fail-closed，不能降级成 `none`。
