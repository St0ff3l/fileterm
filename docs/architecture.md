...
- Rust backend 在 Tauri userData 缺少迁移 marker 时，最多一次导入旧 Electron 用户目录中的应用自有 JSON/SSH key 数据；Tauri 当前数据按 ID 优先，legacy 只补缺失记录，整批 staging/commit 失败会回滚且不写 marker。迁移成功后不再 live merge，Chromium session 与缓存始终不迁移。Windows 便携版（`*-portable.exe`，或 exe 同目录带 `portable` 标记文件）将应用自有存储根目录切换为 exe 旁的 `config`，普通安装版和开发版继续使用 Tauri app-data 目录；第一次切换时若 `config` 为空，会从原 Tauri app-data 复制应用自有 JSON、凭据密文、密钥和字体数据，使用独立 marker 防止重复覆盖，诊断日志和 MCP 运行时描述文件不复制。
