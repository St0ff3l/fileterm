...
- Agent / MCP 设置存于 `UiPreferences.mcpAgent`，只含非敏感策略：连接范围（全部已保存连接 / 选中的已保存连接 / 当前活动会话 / 默认连接）与操作策略（仅只读 / 经确认操作 / 完全授权）。该策略在桌面 bridge 的 action route 前执行；连接列表、会话上下文、传输列表与等待传输也必须按范围过滤，不能只依赖 renderer 隐藏。设置页仅检测本机 PATH 和常见用户安装目录，生成 Claude Code / Codex CLI / OpenCode 的可复制注册命令，不执行客户端、不自动写配置文件。
