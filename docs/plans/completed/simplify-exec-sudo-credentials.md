...
3. FileTerm 主窗口安全输入，可选“保存到连接管理器”；仅在主窗口可见且未最小化时展示。

主窗口隐藏、最小化或 renderer 不可用时，不发送看不见的弹窗，直接返回 `SUDO_PASSWORD_NEEDED` / `SU_PASSWORD_NEEDED`；Copilot 和 MCP 可以向用户询问 sudo/su 密码，并把用户明确提供的值作为单次字段重试。`save_*` 只有和显式密码同时出现才生效。密码通过 SSH stdin 发送，不进入命令文本、结果或日志。
