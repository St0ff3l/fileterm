...
本计划只收录评审发现的、**未被现有 active plan 覆盖**的工作项。renderer `app.tsx` 拆分、workspace service 解耦、TransferService 等已有专项计划，见末尾「与现有 plan 的关系」。
...
- SSH session 实现中的未类型化字段访问全部消除，对应状态由当前 Rust 模块明确维护。
- 渲染端 6 处 `any` prop 替换为具体类型。
- `services/profile_ops/tests.rs` 的 group/parentId 自愈逻辑、`services/workspace/tests.rs` 的终端事件转发有单测覆盖。
...
- `apps/tauri/package.json` devDependencies 无 eslint / prettier（仅 typescript/vite/concurrently 等）。
...
2. 在仓库根新建 `eslint.config.js`（flat config，匹配当前 ESLint 9+ 生态）：
   ...
   - renderer 文件（`apps/tauri/src/renderer/**/*.{ts,tsx}`）启用 `eslint-plugin-react` + `eslint-plugin-react-hooks`（`rules-of-hooks` 必须 error）。
   - Rust backend/bridge 文件禁用 `no-console`（main 有合法日志）。
...
- 迁移前 CI 只执行 Checkout → Setup Node → Install dependencies → Build packages → Typecheck → Build，**没有测试 step**。
- 当前根目录 `package.json` 统一通过 `npm run test:tauri` 执行 Tauri Rust unit/contract 测试。
...
1. 在 `.github/workflows/ci.yml` 的 `validate` job 中，于 "Typecheck" step 之后、"Build" step 之前，新增测试 step：
   ...
   ```
2. Rust contract 与 protocol fixture 统一由 `npm run test:tauri` 运行，避免维护已经移除的分散脚本。
3. CI 失败时按当前 workflow 的日志/构建产物配置上传 artifact。

**验收标准**：

- [x] CI 在每次 push/PR 上执行 `npm run test:tauri`。
- [x] 测试失败时 CI 标红阻断。
- [x] Rust contract/protocol 测试已纳入 Tauri 测试入口。
...
### P1-1. 清理 SSH session 实现中的未类型化字段

**问题**：迁移前 SSH session 实现曾用未类型化字段绕过类型系统；当前 Rust 实现应让会话状态落在明确的模块和结构体中。

**历史证据**（迁移前实现，当前文件已移除）：

- SSH shell 注入、sudo 窗口和近期按键缓冲曾通过动态字段保存。
...
2. 将这些状态迁移为 Rust 结构体字段，并由 `apps/tauri/src-tauri/src/sessions/ssh/` 内的 worker/shell 模块管理。
3. 确认这些字段的初始值与原 `|| 0` / `|| ''` 回退语义一致（声明时给默认值即可）。
4. 跑 `npm run typecheck -w @fileterm/tauri` 确认无新类型错误。

**验收标准**：

- [x] `apps/tauri/src-tauri/src/sessions/ssh/` 不再依赖迁移前动态字段。
- [x] 会话状态由 Rust 模块中的明确类型承载。
- [x] typecheck 通过。

**进阶（可选，独立工作项）**：继续把 CWD 跟踪和 sudo 检测逻辑保持在 `sessions/ssh/shell/` 的职责模块内，避免重新集中到单一控制器。
...
- `apps/tauri/src/renderer/features/workspace/workspace-stage.tsx` 约 156-168 行，命令创建回调 prop 用 `input: any`。
- `apps/tauri/src/renderer/features/workspace/home-workspace.tsx` 约 55-66 行，`tabBarProps: any` 等。

**影响**：组件入参失去类型保护，调用方传错结构不会报错；重构 core 类型时 UI 调用点不会被 TS 追踪到。

**具体改法**：

1. `workspace-stage.tsx`：用 `@fileterm/core` 的 `CommandTemplateInput`（或对应类型）替换 `input: any`；若该回调签名复杂，为该 prop 定义独立接口。
2. `home-workspace.tsx`：为 `tabBarProps` 定义具体接口（可复用 `features/layout/tab-bar.tsx` 已导出的 `OrderedTabEntry` / `TabContextTarget` 等类型组合）。
3. 全仓库 grep `: any` 在 renderer 下的命中，逐个评估是否可用具体类型替换（评审统计 renderer 共约 6 处 any，集中在上述两文件）。

**验收标准**：

- [x] `grep -rn ": any" apps/tauri/src/renderer` 在 props/参数位置零命中（局部 `any` 如事件对象可酌情保留并加注释）。
- [x] typecheck 通过。
...
**问题**：当时测试覆盖严重偏科；当前测试已经按 Rust service/session/command 模块归档，并由 `npm run test:tauri` 统一执行。
...
1. `apps/tauri/src-tauri/src/services/profile_ops/tests.rs`
   - 覆盖 profile 操作的 group/parentId 双向自愈：
...
2. `apps/tauri/src-tauri/src/services/workspace/tests.rs`
   - 覆盖 workspace state 的终端事件顺序、合并和生命周期逻辑。
   - 验证：高频写入被合并、flush 后缓冲清空、定时器生命周期。

**第二批（中优先，需 mock）**：3. `packages/core` 的 `mergeSystemMetricsHistory` 和 `createTabLayout` 纯函数补单测。4. workspace state 的事件转发机制（CWD 变更、Root 状态提权）用 Rust session fixture 做集成测试。
...
- [x] 第一批两个测试文件创建并通过。
- [x] Tauri 测试统一由根脚本 `npm run test:tauri` 调用，覆盖 Rust unit/contract。
...
### P2-1. Workspace service facade

当前 workspace facade 已拆为 `services/workspace/mod.rs`、`model.rs`、`profiles.rs`、`state.rs`，并通过 command/session/transfers 模块协作；`TransferService` 的拆分记录见 `docs/plans/completed/fileterm-core-decoupling-plan.md`。
...
- 2026-07-10：P0 质量门禁三件套全部落地——ESLint+Prettier 配置完成（`--max-warnings=0` 全绿）、CI 接入 `npm test`，husky pre-commit(lint-staged)/pre-push(typecheck) 门禁已建。P1 技术债清理全部完成；profiles 自愈、workspace 终端事件、platform-probe 和 Windows collector 单测已纳入 Tauri 测试入口，四项门禁全绿验证通过。
