...
Copilot 中 sudo/su 的入口统一为：

1. profile 加密存储；
2. FileTerm 主窗口安全输入，可选保存到连接管理器；
3. 如果主窗口/renderer 不可用，Copilot 停止本轮并等待用户明确重试；Copilot 永远不接收或生成一次性密码字段。

半自动和全自动都可以使用本地安全输入；缺少凭据时 FileTerm 会恢复、解除最小化并聚焦主窗口，Copilot 对话区显示等待前台输入的说明，工具活动同步显示状态，工具回合保持等待。密码错误、取消、超时、目标变化、交互输入或其他非成功工具结果都会结束本轮自动工具调用，必须由用户明确重试。
