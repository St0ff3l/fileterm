...
- [x] 从 SSH session runtime 中剥离系统监控采集脚本。
- [x] 在 `apps/tauri/src-tauri/src/sessions/system_metrics/` 建立 `SystemMetrics` 相关采集与解析模块（存储原始指标数据，如字节数、秒数等）。
...
- [x] 实现平台自动探测器 (`sessions/system_metrics/exec.rs::probe_remote_platform`)：在会话建立后首先运行快速探测命令，判定远端主机 OS 类型。
- [x] 在 SSH 系统指标刷新链路中集成该路由；`apps/tauri/src-tauri/src/services/workspace/state.rs` 负责状态广播，SSH worker 根据探测结果调用对应的 Platform Collector。
...
- **平台采集器拆分**：Rust backend 已在 `apps/tauri/src-tauri/src/sessions/system_metrics/` 拆分平台探测、Linux/BusyBox/Windows collector 和 parser，输出 `uptimeSeconds` 与内存/网络等 raw bytes 字段。
...
- 2026-07-09：完成系统监控采集拆分；当前 SSH metrics 通过 `apps/tauri/src-tauri/src/sessions/system_metrics/` 的平台探测路由到 Linux、BusyBox/OpenWrt 或 Windows PowerShell collector。
- 2026-07-10：Windows 兼容性加固——`sessions/system_metrics/parser.rs` 入口统一 `replace(/\r\n?/g, '\n')` 归一化 CRLF 污染；`windows.rs` 新增多级 fallback（CIM 2s → WMI Job 2s → WMIC 进程 2s → .NET API → cmd 工具）与完整性标记 `__FILETERM_METRICS_COMPLETE__` 防半截输出；`sessions/ssh/shell/cwd.rs` 新增 `supportsPosixShellSetup()` fail-closed 双重门控，Windows/unknown 平台不再注入 POSIX CWD 脚本。新增 platform probe、Windows collector 和 parser CRLF 回归测试。
