...
- ✅ 本机最终门禁：Rust lib test 351/351、clippy、Tauri typecheck、lint 和 Prettier 全部通过；新增 sudo/su 三层密码源优先级纯单测，锁定“一次性参数 > profile secret > sudoSameAsLogin（仅 sudo）”且 su 不回退登录密码。
