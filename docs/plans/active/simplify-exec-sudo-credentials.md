...
- 旧 `responseMode=command-proposal` 只允许纯对话兼容路径；半自动 / 全自动的 Provider prompt 不得出现严格命令卡 JSON 指令，只能通过 FileTerm tool loop 产生执行提案。
- ✅ 本机最终门禁：Rust lib test 352/352、clippy、Tauri typecheck、lint 和 Prettier 全部通过；新增 sudo/su 三层密码源优先级纯单测，锁定“一次性参数 > profile secret > sudoSameAsLogin（仅 sudo）”且 su 不回退登录密码。
