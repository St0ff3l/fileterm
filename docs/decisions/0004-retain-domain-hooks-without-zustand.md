...
- 跨窗口、会话和协议状态，以 Rust backend 为真理源，通过 workspace snapshot、领域事件和 Tauri bridge IPC 同步。
...
- Rust backend 继续作为会话、传输和持久化 workspace 状态的唯一真理源。
- 跨窗口状态继续通过 `Rust backend -> bridge -> renderer` 的 snapshot/事件边界同步，不为减少 props 而复制到全局 store。

只有同时满足以下条件时才重新评估 store：

1. 多个非父子 feature 需要直接订阅同一份高频可变状态。
2. props 传递已经跨越多个纯转发层，且组件边界无法合理调整。
3. 引入 store 不会与 Rust backend snapshot 形成双真理源。

## 影响

- 不新增 Zustand 依赖，也不进行无收益的状态迁移。
- 保持当前 hooks 职责清晰、Rust backend 状态权威的架构。
