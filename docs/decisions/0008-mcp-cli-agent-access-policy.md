...
设置页将这两个维度展示为两张独立策略卡片：第一张选择 Agent / MCP / CLI 共用执行权限并提供能力对照，第二张选择“所有连接 / 指定连接”并维护白名单。连接自身的协议能力和 FileTerm 安全校验继续作为更高优先级的硬上限，不新增每条连接的 MCP 专属权限。
...
| 手动一次性 CLI | `fileterm <command>` | 与 MCP 使用同一 FileTerm 主窗口审批                                                           | 仅供用户手动调试和 shell 脚本；外部 Agent 不按请求使用，查询和 Copilot 判定的普通安全命令自动执行 |
| Agent          | `fileterm agent`     | 基础安全操作中的变更、破坏性/提权命令、会话变更、文件/传输变更和隧道进入同一桌面审批队列      | 不能通过请求字段关闭审批                                                                          |
...
一次性 CLI 保留用于 shell 脚本和手动调试，不承诺“零进程”。外部 Agent 的接入契约固定使用常驻 MCP 或 `fileterm agent`，不得为每个动作重新 spawn FileTerm CLI；设置页和 Agent 注册说明必须把一次性 CLI 标为手动/脚本入口。
...
- 用户可以按连接和操作等级限制所有 MCP/CLI/Agent 客户端，而不需要为每个客户端维护一套权限。
...
- 手动一次性 CLI 本身仍是一个进程；要让外部 Agent 复用进程必须使用常驻 Agent 或 MCP。
