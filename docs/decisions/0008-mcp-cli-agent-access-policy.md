...
- `operationPolicy`：只读、基础安全操作或完全访问。

设置页将这两个维度展示为两张独立策略卡片：第一张选择 MCP / CLI 共用执行权限并提供能力对照，第二张选择“所有连接 / 指定连接”并维护白名单。连接自身的协议能力和 FileTerm 安全校验继续作为更高优先级的硬上限，不新增每条连接的 MCP 专属权限。
...
策略在 desktop bridge 的 action route 入口执行，而不是只在 renderer 隐藏按钮。MCP、一次性 CLI 和 CLI JSONL 都使用同一个策略评估器；profile 删除后授权 ID 自动清理，旧配置缺失字段时采用 fail-closed 默认值。

完全访问只跳过逐次操作审批（包括 sudo/su 操作确认）；不能绕过连接范围、session revision、路径校验、凭据边界或 terminal 输入限制。sudo/su 密码仍可能需要输入。

### 2. 三种调用入口具有明确且不互相扩大权限的语义

| 来源           | 入口                     | 审批语义                                                                                      | 备注                                                                                              |
| -------------- | ------------------------ | --------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
| MCP            | `fileterm mcp`           | 基础安全操作中的变更、破坏性/提权命令、会话变更、文件/传输变更、隧道和 sudo/su 进入主窗口审批 | 查询和 Copilot 判定的普通安全命令自动执行，遵循 MCP JSON-RPC 和 progress 通知                     |
| 手动一次性 CLI | `fileterm cli <command>` | 与 MCP 使用同一 FileTerm 主窗口审批                                                           | 仅供用户手动调试和 shell 脚本；外部 Agent 不按请求使用，查询和 Copilot 判定的普通安全命令自动执行 |
| CLI JSONL      | `fileterm cli --jsonl`   | 基础安全操作中的变更、破坏性/提权命令、会话变更、文件/传输变更和隧道进入同一桌面审批队列      | 不能通过请求字段关闭审批；一个进程可处理多个 JSONL 请求                                           |

调用来源只用于审计和选择正确的输出协议，不用于绕过连接范围。CLI JSONL 不区分具体客户端；所有外部 CLI/MCP 客户端共享同一份 FileTerm 全局策略。

### 3. SSH 登录凭据归 FileTerm 主窗口所有

`ConnectionOperation` 只保存 operation ID、profile ID、tab ID 和非敏感状态，并通过 profile-scoped single-flight 去重。MCP/CLI JSONL 的 `open_connection` 默认等待连接就绪，也可以返回 operation ID 后通过 `wait_for_connection` 继续等待。

当 SSH 登录密码、私钥口令或 keyboard-interactive 需要用户输入时：

1. SSH worker 通过现有 `ssh:interaction` 事件请求主窗口输入。
2. desktop bridge 向原始 CLI/MCP/CLI JSONL 调用发送脱敏 progress。
3. 用户完成或取消输入后，原始调用收到最终 Connected 或稳定错误码。

SSH 登录密码不新增到 MCP/CLI 的公开参数、stdout、stderr、结果或日志。普通 exec 仍不接管 MFA、确认、安装器、`passwd` 或 REPL 等通用交互输入。
...
- 用户可以按连接和操作等级限制所有 MCP/CLI/CLI JSONL 客户端，而不需要为每个客户端维护一套权限。
