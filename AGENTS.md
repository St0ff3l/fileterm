...
当前阶段：**骨架 + 核心链路打通 + 分层整理**。质量门禁（ESLint/Prettier/Husky/CI 测试）已落地，四项门禁全绿；App.tsx 从 3716 行瘦至 1274 行（-66%），已拆出 7 个 hooks + ModalPortalManager + ErrorBoundary。
...
- 主入口：`apps/desktop/src/renderer/App.tsx`（1274 行，已拆出 7 个 hooks + 2 个 layout 组件）
...
- `apps/desktop/src/renderer/App.tsx`（1274 行，hooks 编排 + 布局组合）
...
3. `App.tsx` 拆分：7 个 hooks + ModalPortalManager + ErrorBoundary（3716 → 1274 行）
