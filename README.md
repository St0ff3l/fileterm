...
前往 [GitHub Releases](https://github.com/St0ff3l/termdock/releases/latest) 下载最新正式版：
...
TermDock 面向开发者和运维人员的日常远程工作：将远程终端、文件管理、传输任务和连接配置集中到同一个桌面工作区。

- 使用 SSH 时，终端与 SFTP 文件面板自然联动。
- 使用 FTP 时，界面直接进入专注文件操作的工作流。
- 多个连接以标签页并行运行，互不打断。
- 上传、下载、进度与错误统一进入传输任务中心。
- 连接配置、工作区状态和主题体验均为长期使用设计。

正式版聚焦最常用的 `SSH / SFTP / FTP` 工作流：把远程终端、文件操作和传输任务做稳、做顺、做漂亮。
...
| FTP 连接管理 | 使用独立于 SSH 的连接模型，避免协议能力相互耦合。 |
| SSH 终端 | 基于 xterm.js，支持输入输出、自适应尺寸、搜索、剪贴板互通和悬浮命令输入条。 |
| SFTP 文件管理 | 支持远程目录浏览、读写、新建、删除、重命名和权限修改。 |
| FTP 文件管理 | 提供 FTP 会话、远程目录浏览和文件操作能力。 |
| 文件编辑器 | 基于 Monaco Editor 的双栏文件树与编辑区，支持语法高亮、查找替换、编码和语言切换。 |
| 传输任务中心 | 统一管理上传、下载、进度、速度、取消及文件夹递归传输。 |
| 工作区标签 | 支持多标签并行连接、断开、重连、状态持久化和标签切换动效。 |
| 命令模板 | 支持快捷命令、文件夹分组、参数占位符和一键发送。 |
| 主题系统 | 通过 tokens、CSS variables 与组件皮肤实现深色/浅色主题切换。 |
| 远程状态监控 | 展示连接状态与系统资源摘要。 |
| 桌面窗口体验 | 支持连接管理器、命令管理器、文件编辑器独立窗口，以及侧栏、文件抽屉和工作区焦点模式。 |
...
1. 持续提升 `SSH / SFTP / FTP` 主链路的稳定性与可用性。
2. 拆分 `ipc.ts`、`workspace-service.ts`、`session-controllers.ts` 和 `App.tsx`，进一步明确分层边界。
...
如果 TermDock 对你有帮助，欢迎到 [GitHub](https://github.com/St0ff3l/termdock) 点亮 Star：
