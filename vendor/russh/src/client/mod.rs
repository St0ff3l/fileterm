...
impl GexParams {
    ...
    /// Build client parameters for an explicitly opted-in legacy peer.
    ///
    /// RFC 4419 permits a 1024-bit minimum, but the normal client
    /// configuration rejects it because such a group is too weak for a
    /// general-purpose connection. A small number of older network devices
    /// only complete their SHA-1 group-exchange handshake when the client
    /// requests `1024/1024/8192`. Keep this constructor separate so callers
    /// must make that legacy choice explicitly; the secure/default client
    /// constructor remains unchanged.
    pub fn for_legacy_client_config(
        min_group_size: usize,
        preferred_group_size: usize,
        max_group_size: usize,
    ) -> Result<Self, Error> {
        Self::build(
            ...
            preferred_group_size,
            max_group_size,
            ValidationKind::LegacyClientConfig,
        )
    }
        ...
        match kind {
            ...
            ValidationKind::LegacyClientConfig => {
                if self.min_group_size < 1024 {
                    return Err(Error::InvalidConfig(format!(
                        "legacy min_group_size must be at least 1024 bits. We got {} bits",
                        self.min_group_size
                    )));
                }
            }
...
pub struct Config {
    ...
    pub gex: GexParams,
    /// Enable the narrowly scoped Comware legacy GEX compatibility path.
    ///
    /// When enabled, the client requests the legacy 1024/1024/8192 range
    /// only for a matching Comware SSH identification and SHA-1 GEX. Other
    /// peers continue to use the configured GEX parameters.
    pub comware_legacy_gex: bool,
